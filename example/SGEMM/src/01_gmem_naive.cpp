...
struct GMemNaiveKernel
{
    ...
    ALPAKA_FN_ACC void operator()(TAcc const& acc, auto const in1, auto const in2, auto out, float alpha, float beta) const
    {
            ...
            alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid, alpaka::IdxRange{out.getExtents()}))
        {
            ...
            if (row < out.getExtents().x() && col < out.getExtents().y()) {
                float tmp = 0.0;
    ...
    auto computeExec)
{
    ...
    auto frameSpec = alpaka::onHost::FrameSpec{Vec2D{5, 5}, Vec2D{4, 4}};

    std::cout << "Testing GMemNaiveKernel with scalar indices with a grid of " << frameSpec << "\n";
    queue.enqueue(
        computeExec,
        frameSpec,
        GMemNaiveKernel{},
